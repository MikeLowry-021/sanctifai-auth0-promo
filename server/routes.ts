...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/search", async (req, res) => {
    try {
      ...
      // Smart Cache Invalidation: Only return cached results with valid scores (score !== 50)
      // Score of 50 indicates a fallback/error response that should be retried
      if (cached && cached.discernmentScore !== 50) {
        console.log(`[Cache] Returning valid cached analysis for "${title}" (score: ${cached.discernmentScore})`);
      ...
      // If we have a cached result with score 50, we'll retry and update it
      const cachedRecordId = cached?.id;
      if (cachedRecordId) {
        console.log(`[Cache] Found fallback result (score: 50) for "${title}" - retrying analysis...`);
      }
      ...
      // Perplexity will return fallback analysis if API key is missing (service-unavailable tag)
      // Benefits: Real-time web search for current reviews, content warnings, and parent guides
      ...
      // Update existing record if we have a cached fallback, otherwise create new
      const saved = cachedRecordId
        ? await storage.updateAnalysis(cachedRecordId, insertData)
        : await storage.createAnalysis(insertData);

      if (cachedRecordId) {
        console.log(`[Cache] Updated fallback record with new analysis (score: ${analysis.discernmentScore})`);
      }
